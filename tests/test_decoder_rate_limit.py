...
def test_admin_token_required():
    ...
    # ensure no ADMIN_TOKEN set -> open access
    if 'ADMIN_TOKEN' in os.environ:
        del os.environ['ADMIN_TOKEN']
    r = client.get('/decode-login', query_string={'username': 'user', 'password': 'userpass'})
    assert r.status_code == 200

    # set token and ensure 403 without header
...
def test_decoder_rate_limit_memory():
    ...
    # ensure ADMIN_TOKEN is not set so endpoint is accessible
    if 'ADMIN_TOKEN' in os.environ:
        del os.environ['ADMIN_TOKEN']
    app.testing = True
    client = app.test_client()
